export function renderSafetyGate(safetyData, onShowModal) {
  ...
  if (safetyData.approved) {
    ...
    gateBanner.innerHTML = `
      ...
      <button class="action-btn" id="viewRejectionDetailsBtn" style="width: auto; padding: 4px 10px; background: rgba(239, 68, 68, 0.3); border-color: #EF4444; color: #fff;">VIEW REJECTION DETAILS</button>
    `;
...
export function showRejectionModal(safetyData) {
  ...
  if (safetyData.alternate_ports && safetyData.alternate_ports.length > 0) {
    alternateListHtml = `
      <div style="margin-top: 1rem; background: rgba(6, 182, 212, 0.1); border: 1px solid var(--accent-cyan); border-radius: 8px; padding: 1rem;">
        <div style="font-weight: 700; color: var(--accent-cyan); margin-bottom: 0.5rem; font-size: 0.85rem;">RECOMMENDED DEEP-WATER ALTERNATIVE DISCHARGE PORTS:</div>
        <ul style="padding-left: 1.2rem; font-size: 0.82rem; color: #F8FAFC;">
  ...
  modalBody.innerHTML = `
    <div style="background: rgba(239, 68, 68, 0.15); border: 1px solid rgba(239, 68, 68, 0.4); border-radius: 8px; padding: 1rem; margin-bottom: 1rem;">
      <div style="font-weight: 800; color: #F87171; font-size: 0.95rem; margin-bottom: 0.5rem;">🚨 CRITICAL PHYSICAL GATEWAY VIOLATION DETECTED</div>
      <p style="font-size: 0.85rem; color: #F8FAFC; margin-bottom: 0.5rem;">The requested charter does not satisfy Indian Ministry of Shipping & Port Authority physical clearance standards:</p>
    ...
    <div style="font-size: 0.82rem; color: var(--text-secondary);">
      <strong>Directives:</strong> Immediate vessel rejection logged. Downstream vessel recommendations and spot procurement authorizations are suspended until safe discharge port routing or lighterage operations are assigned.
    </div>
    ${alternateListHtml}
  `;
